...
ap.add_argument("-c", "--covers", required=True,
                help="path to the directory that contains our bookcover")
...
quertImage = cv2.imread(args["query"])
gray = cv2.cvtColor(quertImage, cv2.COLOR_BGR2GRAY)
(queryKps, queryDescs) = cd.describe(gray)

results = cv.search(queryKps, queryDescs)

cv2.imshow("Query", quertImage)
